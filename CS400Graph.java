...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Vertex {
		public T data; // vertex label 顶点or application specific data
	...
	/**
	 * Edge objects边缘目标 are stored within their source vertex, and group together
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			// Adding old distance to the new distance
			this.distance = distance + extendBy.weight;
			this.dataSequence = new LinkedList<>();
			
			/*
			 * for (T i : copyPath.dataSequence) {
			 * this.dataSequence.add(copyPath.dataSequence.get((int) i)); }
			 */

			// put copypath's ordered sequence to the list
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		// fails to find any path from the start to end vertices
		...
		// Creating varaibles
		PriorityQueue<Path> Totalpath = new PriorityQueue<Path>();
		// list node being visited
		LinkedList<Vertex> visited = new LinkedList<Vertex>();
		// null list adding the visited at first,remove it after
		LinkedList<Vertex> unvisited = new LinkedList<Vertex>();
		//the list contains the shortest path
		LinkedList<Path> shortestPath = new LinkedList<Path>();
		unvisited.add(vertices.get(start));
		Totalpath.add(new Path(vertices.get(start)));

		//make sure list of path is not empty
		while (Totalpath.isEmpty() == false) {
			Path tempPath = Totalpath.remove();
			// remove node from the unvisited

			unvisited.remove(tempPath.end);
			// every edges
			LinkedList<Edge> everyEdges = tempPath.end.edgesLeaving;
			for (int i = 0; i < everyEdges.size(); i++) {
				Path newPath = new Path(tempPath, everyEdges.get(i));
				// condition: the node is not visited
				if (visited.contains(newPath.end)==false) {
					visited.add(newPath.end);
					unvisited.add(newPath.end);
					shortestPath.add(newPath);
					Totalpath.add(newPath);
					continue;
				}
				// make sure have new path
				if (unvisited.contains(newPath.end)) {
					Totalpath.add(newPath);
				}
				
				for (int j = 0; j < shortestPath.size(); j++) {
					// compare the shortest path to new path
					// set the shorter one in the new path
					if (shortestPath.get(j).distance > newPath.distance
							&& newPath.end.data.equals(shortestPath.get(j).end.data)) {
						shortestPath.set(j, newPath);
					}
				}
			}
		}
		
		//search from the shortlest path list
		for (int i = 0; i < shortestPath.size(); i++) {
			if (end.equals(shortestPath.get(i).end.data)) {
